...
class SyncWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    private fun syncAppUsage() {
        ...
        ApiClient.ingest(applicationContext, "usage", payload)
    }
    ...
    // ---------- Server-dən blok siyahısını al və tətbiq et ----------
    private fun syncBlockedApps() {
        val config = ApiClient.fetchDeviceConfig(applicationContext) ?: return
        // Blok siyahısı BlockEnforcer tərəfindən oxunub tətbiq olunur (VPN/Accessibility servis)
        Prefs.saveBlockedConfig(applicationContext, config.toString())
    }
