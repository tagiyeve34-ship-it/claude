...
object Prefs {
    ...
    // Serverdən gələn son blok siyahısı/geofence konfiqurasiyası (JSON string olaraq)
    private const val KEY_BLOCKED_CONFIG = "blocked_config"
    fun saveBlockedConfig(ctx: Context, json: String) {
        prefs(ctx).edit().putString(KEY_BLOCKED_CONFIG, json).apply()
    }
    fun blockedConfig(ctx: Context): String = prefs(ctx).getString(KEY_BLOCKED_CONFIG, "") ?: ""
}
